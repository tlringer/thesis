...
that can be used to fix other broken proofs broken by the same change, which \sysname defines as a term in \kl{Gallina}.
...
The details of the semantic difference and where differencing looks to find it vary by instance.
Consider a simplified version of the example in Figure~\ref{fig:example}, using the instance for changes in conclusions:
...
\end{itemize}
\end{itemize}
\sysname infers the instance type and the final goal from the change itself, so the proof engineer does not have to provide this information.
\sysname could in theory infer the wrong instance or the wrong goal type, but this would not sacrifice soundness---it would mean only that
