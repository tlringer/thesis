...
The first tool in the \contour[2]{violet}{\kl{\sysnamelong}} \kl{proof repair} \kl{plugin} suite is the namesake \sysnamelong plugin.
...
\sysname implements \kl{proof repair by example}---so called because of its resemblance to programming by example~\cite{DBLP:journals/ftpl/GulwaniPS17}.
...
In this way, proof repair by example is a new form of \kl{proof automation} that accounts for how 
...
This extraction and generalization works at the level of proof terms, through a combination of \kl{semantic differencing} algorithms 
over proof terms (Section~\ref{sec:pumpkin-diff}) and 
semantics-aware \kl{proof term transformations} (Section~\ref{sec:pumpkin-trans}).
\contour[2]{violet}{\kl{\sysname}} automates this process (Section~\ref{sec:pumpkin-impl}).
\kl{Case studies} show retroactively that \sysname could have saved work for proof engineers
...
\sysname also supports changes that are limited in scope in a way that is fundamental to the approach,
...
%clarify solving 2 different problems for 2 chapters, but chapter 4 does better job because was later, could port back to PP but haven't
