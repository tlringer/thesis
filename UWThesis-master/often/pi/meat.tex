...
%\toolnamec moves the reasoning specific to that class of equivalences into the configuration. 
...
term of type \B as if it is an \Aa.
...
\end{lstlisting}
\toolnamec can port proofs across the equivalence between this type and \lstinline{list T}~\href{https://github.com/uwplse/pumpkin-pi/blob/silent/plugin/coq/examples/Example.v}{\circled{3}}. % Example.v
...
\begin{minipage}{0.44\columnwidth}
   \lstinputlisting[firstline=1, lastline=8]{often/pi/nattobin.tex}
\end{minipage}
\hfill
\begin{minipage}{0.54\columnwidth}
   \lstinputlisting[firstline=10, lastline=17]{often/pi/nattobin.tex}
\end{minipage}
\vspace{-0.4cm}
...
constr_ok: $\forall$ $j$ $\vec{x_A}$ $\vec{x_B}$, $\vec{x_A}$ $\equiv_{A \simeq B}$ $\vec{x_B}$ $\rightarrow$
  DepConstr($j$, A) $\vec{x_A}$ $\equiv_{A \simeq B}$ DepConstr(j, B) $\vec{x_B}$.

elim_ok: $\forall$ a b P$_A$ P$_B$ $\vec{f_A}$ $\vec{f_B}$,
...
(@\phantom{constr\_ok: $\forall$ $j$ $\vec{x_A}$ $\vec{x_B}$,}@)
(@\phantom{  DepConstr($j$, A) $\vec{x_A}$ $\equiv_{A \simeq B}$ DepConstr(j, B) $\vec{x_B}$.}@)

iota_ok(A): $\forall$ $j$ P $\vec{f}$ $\vec{x}$ (Q: P(Eta(A) (DepConstr($j$, A) $\vec{x}$)) $\rightarrow$ s),
  Iota(A, j, Q) :
...
%configuring \toolnamec manually, choosing a configuration is important,
...
%The configuration instantiates the proof term transformation to a particular equivalence between \Aa and \B.
...
change, only some of which are useful (for example~\href{https://github.com/uwplse/pumpkin-pi/blob/silent/plugin/coq/playground/refine_unit.v}{\circled{7}}, any \Aa is equivalent to \lstinline{unit} refined by \Aa). % refine_unit.v
...
Fix a configuration. Let \lstinline{f} be the function that uses \lstinline{DepElim} to eliminate \Aa and \lstinline{DepConstr} to construct \B (\lstinline{g} similar). %Assume a univalent metatheory in which equality up to transport is defined.
...
%uses  \lstinline{DepElim} to eliminate \B and \lstinline{DepConstr} to construct \Aa.
...
%Furthermore, since CIC$_{\omega}$ is constructive, the \textit{only} way to construct an \Aa (respectively \B) is to use its constructors,
%and the \textit{only} way to eliminate an \Aa (respectively \B) is to apply its eliminator.
%Finally, since these form an equivalence, all ways of constructing or eliminating \Aa and \B are covered by these dependent constructors and %eliminators.
%So, as long as we are able to unify subterms with applications of \lstinline{DepConstr} and \lstinline{DepElim},
%\textsc{Dep-Constr} and \textsc{Dep-Elim} should preserve correctness of the transformation and cover all values and eliminations of \Aa and \B.
...
\toolnamec implements four search procedures for automatic configuration~\href{https://github.com/uwplse/pumpkin-pi/blob/silent/plugin/src/automation/lift/liftconfig.ml}{\circled{6}};
