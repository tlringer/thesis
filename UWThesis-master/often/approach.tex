...
With \kl{manual configuration}, the proof engineer extracts and generalizes the information corresponding to the change, but \toolnamec can still apply it
...
When a type equivalence between types \Aa and \B exists, those types are \kl{equivalent} (denoted \Aa $\simeq$ \B). % for example:
...
\begin{minipage}{0.44\textwidth}
   \lstinputlisting[firstline=1, lastline=7]{often/listtovect.tex}
\end{minipage}
\hfill
\begin{minipage}{0.55\textwidth}
   \lstinputlisting[firstline=8, lastline=14]{often/listtovect.tex}
\end{minipage}
...
\toolnamec repairs proofs in response to these changes by implementing a kind of proof reuse known as \intro{transport} from \kl{homotopy type theory},
...
Instead, the \toolnamec transformation assumes a univalent \textit{metatheory} in which to interpret its specificaton,
