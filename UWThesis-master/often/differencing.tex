...
In fact, it has formal meaning: the configuration corresponds to an \textit{initial algebra}~\cite{nlab:initial_algebra_of_an_endofunctor},
...
\begin{figure}
TODO
\caption{The initial algebra corresponding to a configuration for equivalent types \Aa and \B.}
\label{fig:lambek}
\end{figure}

But the configuration is in fact more general than that---it can support any two equivalent types \Aa and \B.
This is because \textit{any} equivalent \Aa and \B correspond to an initial algebra (by Lambek's theorem).
Figure~\ref{fig:lambek} shows this initial algebra for an arbitrary configuration for an equivalence between \Aa and \B.
Here, $F A$ is the shape of \Aa (and similarly for \B).
\lstinline{DepConstr} shows up explicitly in the diagram;
\lstinline{DepElim} is the corresponding arrow over the universal predicate lifting (not pictured).

For example, fixing \Aa as \lstinline{nat} and \B as \lstinline{N}, $F A$ is \lstinline{1 + nat}:
the sum of the unit type and \lstinline{nat} (and similarly for \B).
Going from $F A \rightarrow A$, the left injection maps to the \lstinline{0} constructor, and the right injection maps to the \lstinline{S} constructor;
the eliminator for \lstinline{nat} corresponds to the same injections over the universal predicate lifting.
Going from $F B \rightarrow B$, the corresponding injections map not to constructors of \lstinline{N},
but to the terms used to define \lstinline{DepConstr} over \lstinline{N};
the dependent eliminator used to define \lstinline{DepElim} over \lstinline{N} corresponds to the same injections over the universal predicate lifting.

\lstinline{Eta} and \lstinline{Iota} both show up implicitly in the diagram, thanks to the nuances of definitional equality not captured by the categorical representation.
In particular, the proof that \lstinline{Eta} corresponds to the identity function is used in the proof
that $f$ is unique up to isomorphism---to see this in the most basic case, let \B be \Aa.
\lstinline{Iota} is used in the proof that the square commutes---this is most clear over the universal predicate lifting.
