...
This chapter presents the \intro{\toolnamec} extension to the \kl{\sysnamelong} proof repair plugin suite.\footnote{I annotate each claim to which code is relevant with a circled number like \circled{1}. These circled numbers are links to code, and are detailed in \href{https://github.com/uwplse/pumpkin-pi/blob/v2.0.0/GUIDE.md}{\lstinline{GUIDE.md}}.} % TODO update links
\toolnamec is a \kl{plugin} that adds support for proof repair across a broad class of changes in datatypes called 
\textit{type equivalences} (Section~\ref{sec:pi-scope}),
thereby supporting a large class of practical repair scenarios.
Proof repair \kl{across type equivalences} with \toolnamec makes progress on two challenges that the \sysname prototype had left open:
...
As the \kl{\textsc{REPLica}} user study showed, none of these were informed by the current needs of proof engineers. %~\cite{replica}. TODO elaborate?
...
to refer only to the new version, and how to do so in a way that does not break \kl{definitional equality}.
