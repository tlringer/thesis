...
Consider inverting an arbitrary sequence of two rewrites (again in Coq rather than CIC$_{\omega}$): % TODO just use CIC here and above 

\begin{lstlisting}
  t : X $\rightarrow$ Z := (@\diff{eq\_ind\_r G ...}@) ((@\diff{eq\_ind\_r F ...}@)).
\end{lstlisting}
We can view \lstinline{t} as a term that composes two functions:

\begin{lstlisting}
  g : Y $\rightarrow$ Z := (@\diff{eq\_ind\_r G ...}@)(@\vspace{-0.04cm}@)
  f : X $\rightarrow$ Y := (@\diff{eq\_ind\_r F ...}@)(@\vspace{-0.04cm}@)
  t : X $\rightarrow$ Z := g $\circ$ f.
\end{lstlisting}
The inverse of \lstinline{t} is the following:

\begin{lstlisting}
  t$\inv$ : Z $\rightarrow$ X := f$\inv$ $\circ$ g$\inv$.
\end{lstlisting}
...
The lemma factoring component looks within a term for its factors.
...
\paragraph{Undecidability} (Needs rewording but draft text, whatever.)
