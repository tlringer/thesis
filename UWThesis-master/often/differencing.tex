...
and proves a \kl{type equivalence}. But differencing further decomposes that equivalence into a form called a \kl{configuration}. 
...
\lstinline{DepConstr} and \lstinline{DepElim} define how to transform \intro{dependent constructors} and \intro{dependent eliminators}, thereby preserving the equivalence, and 
...
and \lstinline{DepElim} (\kl{dependent constructors} and \kl{dependent eliminators}).
...
even if \Aa and \B themselves are \kl{inductive types} with different numbers of constructors
...
cases over $\Sigma$\lstinline{(n : nat).vector T n}. % \lstinline{packed_vect}. %packs constructors and eliminates projections.
...
Furthermore, both examples use \kl{automatic configuration}, so differencing in \toolnamec's \textbf{Configure} component
...
in particular by \kl{transporting} equalities.
...
Each \lstinline{Iota}---one per constructor---describes and proves the behavior of $\iota$-reduction
for \lstinline{DepElim} on the corresponding case.
...
To transform proofs about \lstinline{nat} to proofs about \lstinline{N}, we must transform \kl{definitional} \kl{$\iota$}-reduction over \lstinline{nat} to 
\kl{propositional} \kl{$\iota$}-reduction over \lstinline{N}.
...
In particular, the analogy tying the \kl{configuration} to \kl{constructors} and \kl{eliminators}
has meaning in terms of what in category theory are known as \intro{initial algebras of endofunctors},
and correctness---that configurations and equivalences are isomorphic---follows by Lambek's 
theorem (Section~\ref{sec:algebra}).
A more syntactic version of this claim can be used to specify and prove correctness of configurations in a \kl{univalent} metatheory,
or on an ad hoc basis in \kl{CIC$_{\omega}$} (Section~\ref{sec:correct-cic}).

\paragraph{Historical Note}
When I met with \kl{Michael Shulman} over coffee a few years ago, he said that a preliminary version of this work ``feels like univalence,'' ``feels like coherence,'' and ``feels like an endofunctor.'' All three were correct! But I did not understand the connection to endofunctors by way of Lambek's until a few months before writing this thesis.
...
Here, $F\ A$ is the inductive structure of \Aa.
...
parts that talk about \Aa and \B separately. All the transformation in Section~\ref{sec:pi-trans} does is follow the arrows
in the diagram to get from \Aa directly to \B.
But in order to do that, it needs to handle the nuances of \kl{definitional equality} and \kl{dependent types} in \kl{CIC${_\omega}$}---for example,
...
To preserve the equivalence (Figure~\ref{fig:spec}, top), together \lstinline{DepConstr} and \lstinline{DepElim} must form an equivalence
...
where the new index is fully determined by a fold over \Aa (I call this fold the \intro{indexer}). 
...
In Section~\ref{sec:pi-results}, I will show you a case study moving from lists to length-indexed vectors.
Nominally this works by porting functions and proofs along the equivalence from Section~\ref{sec:pi-scope},
but in practice this works by chaining two automatic configurations with some human input.
The first configuration uses a search procedure that discovers the equivalence between lists and vectors of \textit{some} length above,
...
The first configuration nicely demonstrates how differencing works, so let us look at it in detail.

\paragraph{Differencing for Algebraic Ornaments}
Assume \kl{inductive types} \Aa and \AI, related by an \kl{algebraic ornament} with the index of type \I.
In the scope of this thesis, further assume that \Aa and \I are not indexed (\kl{dependent}) types.\footnote{The original paper that this is from lets all of \Aa,
...
for those who are \textit{sufficiently fanatical}.}
Then there is a \kl{type equivalence}:
...
since no lists would map to the vectors of odd lengths.
...
Differencing generates the \lstinline{indexer} by traversing the types of the eliminators for \Aa\ and \AI\ in parallel using the algorithm from Figure~\ref{fig:searchindexer},
...
Finally, \textsc{F-Ind} not only recurses into each case, but also packs the result into an \kl{existential}.
...
To derive \kl{\lstinline{DepConstr}} over \B, differencing takes each constructor of \Aa,
...
replacing them instead with the projections $\pi_l$ and $\pi_r$.

Deriving \kl{\lstinline{DepElim}} over \B works similarly, except that it lifts not just the hypotheses of types \Aa, but 
...
Differencing discovers \kl{\lstinline{Eta}} and \kl{\lstinline{Iota}} directly.
For any \kl{algebraic ornament}, \lstinline{Eta} is the standard $\eta$-expansion for $\Sigma$ types:
...
as well as \lstinline{coherence}.
This proves the correctness property that the configuration induces an equivalence, thereby
increasing confidence in the output of differencing.
The proof of \kl{coherence} follows by reflexivity, thanks to the construction of \lstinline{f} 
