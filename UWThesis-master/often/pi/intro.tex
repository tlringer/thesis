...
The case studies in Section~\ref{sec:search}---summarized in Table~\ref{fig:changes} on page~\pageref{fig:changes}---show that \toolnamec is flexible enough to support
...
For example, \toolnamec is the only proof repair tool we are aware of that produces suggested tactic scripts for repaired proofs,
...
\includegraphics[width=\columnwidth]{often/pi/workflowa}
\vspace{-0.7cm}
...
%We elaborate on some of the examples from Table~\ref{fig:changes} to further demonstrate flexibility and usability with four case studies (Section~%\ref{sec:search}), which show that \toolnamec 1) can support a benchmark from a user study of Coq proof engineers, 2) can simplify dependently-typed programming, %automating manual steps from previous work,
