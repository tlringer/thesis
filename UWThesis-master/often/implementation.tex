...
To handle terms that use these features, \toolnamec includes a \lstinline{Preprocess} command that translates  % TODO link to repo
these terms into corresponding eliminator applications.\footnote{My collaborator Nate Yazdani wrote this command, and it is available as a standalone plugin on Github. % TODO link
...
The code denotes these explicitly~\href{https://github.com/uwplse/pumpkin-pi/blob/v2.0.0/plugin/src/automation/lift/lift.ml}{\circled{4}}
...
Section~\ref{sec:second} explains how \kl{RanDair} scales this to the implementation.
...
While the mini decompiler is very simple, only a few small changes are needed for \kl{RanDair}
to move this to Coq.
