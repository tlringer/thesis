...
\iffalse
\begin{figure}
\lstinputlisting[firstline=1, lastline=18]{often/pi/replica.tex}
\vspace{-0.4cm}
...
\begin{minipage}{0.48\textwidth}
   \lstinputlisting[firstline=1, lastline=9]{often/pi/replica.tex}
\end{minipage}
\hfill
\begin{minipage}{0.48\textwidth}
   \lstinputlisting[firstline=10, lastline=18]{often/pi/replica.tex}
\end{minipage}
\vspace{-0.4cm}
...
In \href{https://github.com/uwplse/pumpkin-pi/blob/master/plugin/coq/nonorn.v}{\circled{5}}, we used \toolnamec to support a classic example of changing inductive structure:
...
In the style of the classic example~\cite{magaud2000changing}, we used \toolnamec to derive a slow binary
...
%\end{enumerate}
%\item \toolnamec was flexible. Each search procedure for automatic configuration
...
%\item \toolnamec had good enough workflow integration to support real use cases.
%The tactic decompiler showed promising early results with clear paths to improvements.
%Some workflows were unanticipated and informed design changes in \toolnamec.
%\end{enumerate}
...
\hfill
\begin{minipage}{0.49\columnwidth}
   \lstinputlisting[firstline=11, lastline=19]{often/pi/replica.tex}
\end{minipage}
\vspace{-0.4cm}
\caption{A simple language (left) and the same language with two swapped constructors and an added constructor (right).}
\vspace{-0.1cm}
...
We used \toolnamec to automatically configure the proof term transformation to swap these constructors,
then repair all of the functions and proofs.
We also succeeded at more difficult variants of this,
like swapping constructors with the same type, renaming all of the constructors,
permuting more than two constructors,
or permuting and renaming constructors at the same time.
In all cases, with a bit of human guidance, \toolnamec repaired the functions and proofs.

We then extended the inductive type to add the new constructor, and used \toolnamec to repair terms,
...
We used \toolnamec to automatically update the functions and proofs in \href{https://github.com/uwplse/pumpkin-pi/blob/master/plugin/coq/Swap.v}{\lstinline{Swap.v}} from the \textsc{Replica} benchmark.
...
%\end{lstlisting}
\toolnamec automatically updated all of these. For the inductive proof, it produced
...
Since the functions \toolnamec produced were guaranteed to preserve the behavior of the old functions,
...
The swap change alone was simple enough that \toolnamec would not have been necessary
...
More advanced variants of this benchmark that we tried did necessitate \toolnamec,
and \toolnamec worked well for those. % TODO what happens with the tactic decompiler for these?
The entire \lstinline{Swap.v} file, which includes swapping constructors of every function in the \lstinline{list} module and
its dependencies, four variants of the \textsc{Replica} swapping change,
and testing a large and ambiguous permutation of a 30 constructor \lstinline{Enum},
took \toolnamec less than 90 seconds total. % TODO specs, reproduction
...
The proof term transformation in \toolnamec is based on the proof term transformation from
\textsc{Devoid}, an earlier version of \toolnamec.
\textsc{Devoid} supported proof reuse across \textit{algebraic ornaments}, which describe relations
...
With \toolnamec, we were able to not only support algebraic ornaments as in \textsc{Devoid},
...
%, though we do not yet know if the latter two ended up using \toolnamec. % TODO honestly ask
...
The updated \toolnamec example automates this last step.

%The workflow for this was a bit different than it was with \textsc{Devoid}.
To handle this, we used a custom eliminator \toolnamec generated to combine the list functions
...
%\toolnamec implemented some additional automation to generate eliminators that help separate out this additional information
%for repair. 
...
We have realized a classic example~\cite{magaud2000changing} of this with \toolnamec using a manual configuration:
...
This allows for a fast addition function, found in the Coq standard library.
In the style of the classic example~\cite{magaud2000changing}, we use \toolnamec to derive a slow binary
addition function that does not refer to \lstinline{nat}.
...
inductive structure from \lstinline{nat}, and is noted as far back as 2000~\cite{magaud2000changing}.
This corresponds to a broader pattern---it captures the essence of the change in inductive structure.
\toolnamec's configurable proof term transformation captures that intuition.
...
Like the classic example~\cite{magaud2000changing}, we found it easy to manually prove that
...
We then used \toolnamec again to transform a proof:
\begin{lstlisting}
...
into explicit applications of \lstinline{Iota} over \Aa.
...
The file took under a second for us to compile using \toolnamec.
...
An industrial proof engineer at Galois has been using \toolnamec in proving
correct an implementation of the TLS handshake protocol.
While this is ongoing work, thus far,
\toolnamec has helped \company integrate Coq with their existing verification workflow.
...
A proof engineer at \company has used \toolnamec to work with those automatically generated programs and specifications
with the following workflow:

\begin{enumerate}
\item use \toolnamec to update the automatically generated functions and specifications into more
...
\item use \toolnamec again to update those proofs about human-readable functions and specifications back to
...
\begin{minipage}{0.33\textwidth}
   \lstinputlisting[firstline=1, lastline=10]{often/pi/records.tex}
\end{minipage}
\hfill
\begin{minipage}{0.66\textwidth}
   \lstinputlisting[firstline=12, lastline=21]{often/pi/records.tex}
\end{minipage}
\vspace{-0.5cm}
...
Some example proofs that the proof engineer used \toolnamec for
...
The proof engineer used \toolnamec to port anonymous tuples produced by \company' compiler
...
%It set \Aa to be the record type and \B to be the tuple.
...
which \toolnamec generated and proved automatically.
...
and then used \toolnamec to automatically port these back to proofs about the original function:
...
The proof engineer was able to use \toolnamec to integrate Coq into an existing proof engineering
workflow using solver-aided tools at \company.
The workflow for using \toolnamec itself was a bit nonstandard,
...
\toolnamec was usable enough for this to work, but we found two challenges with workflow integration:
the proof engineer sometimes could not distinguish between user errors and bugs in our code,
and the proof engineer typically waited only about ten seconds at most for \toolnamec to return.
Both observations informed improvements to \toolnamec, like better error messages, caching of transformed subterms,
and the ability to tell \toolnamec not to $\delta$-reduce certain terms.
% TODO how long did compiling the file take?
\fi
