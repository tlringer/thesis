...
Proof engineering as defined in the survey paper considers proof assistants that satisfy the \textit{de Bruijn criterion}~\cite{Barendregt2002,Barendregt2351}, which requires that they produce proof objects that a small proof-checking kernel can verify.
This includes Coq~\cite{coq}---the proof assistant that this thesis focuses on---but also other proof assistants like
Isabelle/HOL~\cite{isabelle}, HOL Light~\cite{hollight}, Agda~\cite{agda}, Lean~\cite{lean}, and NuPRL~\cite{nuprl}.
These proof assistants have different notions of proof objects (in the case of Coq, proof terms in \kl{Gallina}),
and different interfaces (if any) for proof scripts built on top of those proof objects (in the case of Coq, sequences of tactics in \kl{Ltac}).
...
% Most notably, proof terms.
%But also differences in logics (constructivism, then refer back to it later).

\paragraph{Proof Scripts} %Like different tactic languages.
