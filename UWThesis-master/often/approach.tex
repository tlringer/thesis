...
To configure \toolnamec manually, the proof engineer can pass the equivalence to \toolnamec's \lstinline{Configure repair} command before invoking \lstinline{Repair}.
...
\item \lstinline{section} and \lstinline{retraction} prove the equivalence between \lstinline{f} and \lstinline{g}, and
...
and proves that every configuration corresponds to an equivalence, and similarly in the opposite direction.
Manual configuration takes in the configuration directly.
In either case, the transformation uses this configuration to automatically repair proofs in response to the change
from \Aa to \B that the equivalence correponds to.
