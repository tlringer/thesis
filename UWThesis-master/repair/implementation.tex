...
The source code of the \kl{\sysnamelong} proof repair plugin suite is on Github.\footnote{\url{http://github.com/uwplse/PUMPKIN-PATCH}} % TODO thesis release
It includes the source code of the \kl{\sysname} prototype plugin, extended with a number of new features,
...
This representation is useful for understanding how differencing works over inductive types, but it implementing it adds clutter that makes \sysname difficult to maintain.
...
and one that applies $\delta$ reduction to unwrap constants.
...
\paragraph{Generalization} Generalization (\lstinline{abstraction.ml}) % TODO rename
...
In particular, \sysname is implemented as a Coq plugin,
...
Since releasing the \sysname prototype, I have extended \sysname with many features
...
\item \textbf{\sysname was fast, but it could be even faster.} \\
...
\begin{lstlisting}[language=coq]
  gt_le_S n m :=
    (fun (H : (@\ltacforall@) n0 m0, (@\diff{n0 < m0}@) -> (@\diff{S n0 <= m0}@)) => H n m)
    $\ldots$
...
\begin{lstlisting}[language=coq]
  gt_le_S$\inv$ n m :=
    (fun (H : (@\ltacforall@) n0 m0, (@\diff{S n0 <= m0}@) -> (@\diff{n0 < m0}@)) => H n m)
    $\ldots$
...
\paragraph{A Challenge for Generalization} Generalization produced an exponential number of candidates when generalizing a patch candidate with this type:
...
the later \toolnamec extension in Chapter~\ref{chapt:pi} can support large proof developments in an order of seconds.
