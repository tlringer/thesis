...
%  \STATE Build trees for search
  %\STATE build trees for proofs
  %\REPEAT
    \STATE \textit{diff} \diff{conclusions of the theorems} of old\_proof and new\_proof for goals
    \STATE \textit{diff} \diff{bodies of the proof terms} for candidates
    \STATE \textbf{if} there are candidates \textbf{then}
    \STATE \hspace*{1em} \textit{abstract} and then \textit{specialize} candidates
\end{algorithmic}
\end{algorithm}

was close, now we want a real patch though.
